...
function getPrice(priceStart, priceConstant, priceExponent, elapsedSeconds) {
    var decayRate = Math.floor(Math.pow(elapsedSeconds, priceExponent) /
        priceConstant);
...
function getCurrentPrice(priceStart, priceConstant, priceExponent,
    elapsedSeconds) {
    var decayRate = Math.pow(elapsedSeconds, priceExponent) / priceConstant;
    var price = priceStart * (1 + elapsedSeconds) /
        (1 + elapsedSeconds + decayRate);
    return price;
}
...
function getStaticData() {
    $.when($.getJSON('static.json'), $.getJSON('status.json'))
        .done(function(staticRes, statusRes) {
            ...
            $('.decay-factor').text(json.decayFactor);
...
function handleStageChange(newStage) {
    if (oldStage !== newStage) {
        ...
        switch (newStage) {
            case 'not-started':
                $('#auction-title').text(
                    'The Raiden token auction starts on October 18th around 3pm UTC.');
                $('#auctionrunning').hide();
                ...
                $('#auction-subtitle').text(
                    'The auction format combines certainty of participation with certainty of valuation.'
                );
                $('.not-started').show();
                break;
            case 'started':
